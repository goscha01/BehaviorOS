...
class QuoAdapterFixtureTests(SimpleTestCase):
    def test_fixture_backend_yields_expected_scenarios(self):
        adapter = QuoAdapter(sigcore_url='', sigcore_api_key='')
...
class QuoAdapterBackendSelectionTests(SimpleTestCase):
    ...
    def test_empty_sigcore_url_uses_fixture_backend(self):
        ...
        self.assertGreater(len(list(adapter.fetch_records(limit=1))), 0)

    @override_settings(
        SIGCORE_URL='https://sigcore.example',
        SIGCORE_API_KEY='sc_test_workspace_key',
    )
    def test_sigcore_selected_when_url_and_key_present(self):
        # Both URL and API key must be set to switch to the HTTP backend.
        # The workspace-scoped `sc_` key auto-scopes queries — no separate
        # workspace ID needed.
        adapter = QuoAdapter()
        self.assertTrue(adapter._sigcore_url)  # noqa: SLF001
        self.assertTrue(adapter._sigcore_api_key)  # noqa: SLF001
